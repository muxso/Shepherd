...
//!
//! 匹配维度取自定义(method + path),响应取自 mock(status + body)。直读 api-definition
//! 上下文的邻域表(与 api-test 直读 ms_environment 同构:跨上下文只读、不引 crate 依赖)。
...
impl MockRuleSource for PgMockRuleSource {
    async fn active_rules(&self) -> Result<Vec<MockRule>, SourceError> {
        let rows = sqlx::query(
            "SELECT m.id, d.method, d.path, m.response_status, m.response_body \
