...
fn cmd_err(e: SkillCmdError) -> Response {
    match e {
        ...
        SkillCmdError::Compose(_) => (StatusCode::CONFLICT, "skill composition error (unknown include or cycle)").into_response(),
        SkillCmdError::Repo(_) => (StatusCode::INTERNAL_SERVER_ERROR, "storage error").into_response(),
    }
}
...
async fn create_skill(user: AuthUser, State(st): State<SkillState>, Json(b): Json<CreateBody>) -> Response {
    ...
    match st.create.execute(&b.project_id, &b.name, &b.description, &b.instructions, &b.includes).await {
        Ok(s) => (StatusCode::CREATED, Json(SkillResponse::from(&s))).into_response(),
        Err(CreateSkillError::NameAlreadyExists) => (StatusCode::CONFLICT, "name already exists").into_response(),
        Err(CreateSkillError::Validation(_)) => (StatusCode::BAD_REQUEST, "invalid skill").into_response(),
        Err(CreateSkillError::Repo(_)) => (StatusCode::INTERNAL_SERVER_ERROR, "storage error").into_response(),
    }
}
...
async fn update_skill(
    ...
    match st.admin.update(&id, &b.name, &b.description, &b.instructions, b.includes, b.enabled).await {
        Ok(s) => (StatusCode::OK, Json(SkillResponse::from(&s))).into_response(),
        Err(e) => cmd_err(e),
    }
}
...
async fn compose(user: AuthUser, State(st): State<SkillState>, Json(b): Json<ComposeBody>) -> Response {
    ...
    match st.admin.compose(&b.project_id, &b.skill_ids).await {
        Ok(c) => (StatusCode::OK, Json(ComposeResponse { skill_ids: c.skill_ids, instructions: c.instructions })).into_response(),
        Err(e) => cmd_err(e),
    }
}
...
mod tests {
    ...
    async fn app_with(perms: &str) -> (Router, String) {
        ...
        let r = router(
            CreateSkillUseCase::new(repo.clone()),
            SkillService::new(repo),
            sessions,
        );
        (r, token)
    }
    ...
    async fn create_compose_flow() {
        ...
        let a = app.clone().oneshot(req("POST", "/skill", r#"{"projectId":"p1","name":"基础","instructions":"遵循六边形"}"#, Some(&t))).await.expect("r");
    ...
    async fn compose_cycle_is_conflict() {
        ...
        app.clone().oneshot(req("PUT", &format!("/skill/{b}"), &format!(r#"{{"name":"B","instructions":"b","includes":["{a}"]}}"#), Some(&t))).await.expect("r");
        let c = app.oneshot(req("POST", "/skill/compose", &format!(r#"{{"projectId":"p1","skillIds":["{a}"]}}"#), Some(&t))).await.expect("r");
    ...
    async fn rbac_create_requires_add() {
        ...
        assert_eq!(
            app.oneshot(req("POST", "/skill", r#"{"projectId":"p1","name":"x","instructions":"y"}"#, None)).await.expect("r").status(),
            StatusCode::UNAUTHORIZED
        );
        let (app, t) = app_with("SKILL:READ").await;
        assert_eq!(
            app.oneshot(req("POST", "/skill", r#"{"projectId":"p1","name":"x","instructions":"y"}"#, Some(&t))).await.expect("r").status(),
