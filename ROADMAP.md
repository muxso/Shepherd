...
| C5 | **生产化收尾** | ✅ CORS(`SHEPHERD_CORS_ORIGINS` 允许列表)+ 每客户端令牌桶限流(opt-in `SHEPHERD_RATE_LIMIT_RPS`,429 + Retry-After);容器 / Helm / 多云 Terraform / CI-CD 已交付;metrics 见 A1。待补:统一错误体(problem+json)、分环境配置 | ~1–2 周 | 中 |
