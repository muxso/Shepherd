...
| A6 | **需求/任务广度** | ✅ 依赖图可视化数据(`GET /decomposition/{id}/graph`:节点含拓扑层/就绪态 + 有向边,松弛求层不依赖行序) + ✅ 任务批量重指派(`POST /decomposition/{id}/reassign`:把某执行者的未完成任务整体转给另一个,跳过已完成) + ✅ 通用评论上线(`/comment` 多态挂 REQUIREMENT/BUG/FUNCTIONAL_CASE… + 会话作者 + 软删 + COMMENT RBAC;含需求评论) + ✅ project 成员管理(`/project/{id}/member` 增改/列/移除,OWNER/MEMBER/VIEWER 角色 upsert,PROJECT:UPDATE 管理 / READ 查看) + ✅ 需求手工排序(`PUT /requirement/order`:按 orderedIds 写显式秩,列表 `ORDER BY sort_order, seq`,未排序回落插入序);待补:需求附件 | ~1 周 |
...
| dashboard | 38 / 2 / 36 | 中(聚合查询) | **~5** | 已落:`GET /requirement/summary`(需求按状态)+ `GET /decomposition/{id}/summary`(任务按状态);均域内聚合、可单测 |
