...
顶栏的铃铛打开消息中心:站内消息来自真实事件(缺陷指派 / 状态变更、评审创建、
评论里 `@某人`、定时任务失败),按分类归组,带 全部 / @我的 / 未读 / 已读
页签和一键全部已读。

推送在 项目 → 消息管理 里按项目配置:

- **机器人** —— 飞书、钉钉、企业微信三种 webhook 机器人:填名称和 webhook
  地址;钉钉可选填「加签」密钥(留空则不签名)。每个机器人都有测试发送按钮,
  配规则前先验证 webhook 通不通。
- **通知规则** —— 把事件类型(或通配符 `*`)映射到渠道:站内收件箱和/或若干
  机器人,可选消息模板(`${title}`、`${operator}`、`${time}`;留空用默认文案)。
  规则可以停用而不必删除。
