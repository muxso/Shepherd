...
import { Button, Card, Form, Input, InputNumber, Switch, Spin, Alert, Divider } from 'antd'
...
// System-level RAG config: embedding + chat endpoints/models/keys, retrieval depth and rerank.
    ...
    <div style={{ maxWidth: 720, margin: '0 auto', padding: '24px 16px' }}>
      ...
      >
        <Alert
          ...
          message={t(
            'rag.hint',
            '用于知识问答(聊天)与需求 AI 评审的检索/生成模型。留空 API Key 表示沿用已保存的密钥。',
          )}
        />
        <Form form={form} layout="vertical" requiredMark={false}>
          ...
          <Divider orientation="left">{t('rag.chatSection', '生成模型(Chat)')}</Divider>
          <Form.Item
            name="chatUrl"
            label={t('rag.chatUrl', '生成接口地址')}
            rules={[{ required: true, message: t('rag.required', '必填') }]}
          >
          ...
          <Form.Item
            name="maxTokens"
            label={t('rag.maxTokens', '生成最大 Token')}
            rules={[{ required: true, message: t('rag.required', '必填') }]}
          >
            <InputNumber min={1} max={32768} style={{ width: '100%' }} />
          </Form.Item>

          <Divider orientation="left">{t('rag.retrievalSection', '检索')}</Divider>
          <Form.Item
            name="topK"
          ...
          <Form.Item name="rerank" label={t('rag.rerank', 'LLM 重排')} valuePropName="checked">
            <Switch />
          </Form.Item>
